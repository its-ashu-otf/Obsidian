# SSH Fundamentals and Enumeration

![[Pasted image 20250430221401.png]]

## Basic Concepts

- SSH (Secure Shell) is an encrypted remote administration protocol that replaced Telnet
- Primarily used for secure remote access to servers and systems
- Default TCP port: 22 (can be configured to use other ports)
- Can be targeted for enumeration and brute-force attacks during penetration testing
...
# SSH-specific script scanning
nmap -p 22 --script ssh-auth-methods,ssh-hostkey,ssh-brute <target>
```

### Metasploit Modules

- `auxiliary/scanner/ssh/ssh_version` - Identifies SSH version
- `auxiliary/scanner/ssh/ssh_login` - Performs brute-force attacks
- `auxiliary/scanner/ssh/ssh_enumusers` - Attempts to enumerate valid users

### Common Vulnerabilities

- CVE-2018-10933 (libssh authentication bypass)
- Outdated SSH protocol versions (1.x is considered insecure)
- Weak ciphers or MAC algorithms
- SSH key misconfigurations

### Brute-Force Considerations

- Be mindful of account lockout policies
- Use appropriate timing to avoid detection
- Consider using curated wordlists for higher success rates

## Additional Tools

- Hydra: Fast network authentication cracker
- Medusa: Parallel login brute-forcer
- Patator: Multi-purpose brute-forcer with SSH module
