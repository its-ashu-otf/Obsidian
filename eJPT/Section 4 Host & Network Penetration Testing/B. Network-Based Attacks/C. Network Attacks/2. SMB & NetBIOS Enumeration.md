...
---
## 🔍 **NetBIOS vs SMB - Overview**

### 🧠 **NetBIOS (Network Basic Input/Output System)**

- **Type:** Legacy API + Protocol
    
- **Purpose:** Provides **name resolution** and **communication services** over LAN.
    
- **Used by:** Applications for **network discovery** and communication in **pre-DNS** era.
...
|---|---|
|📛 Name Service (NBNS)|Registers/resolves NetBIOS names. UDP/137|
|📢 Datagram Service|Connectionless message broadcasting. UDP/138|
|🔗 Session Service|Reliable connection-oriented communication. TCP/139|
...
- **Purpose:** Enables **file, printer, and resource sharing** on networks.
    
- **Used by:** Windows systems primarily, also implemented in _Samba_ (Linux).
...
|---|---|
|SMB 1.0|Old, insecure (used by WannaCry); **Disable it!**|
|SMB 2.x|Better performance, reduced overhead (Windows Vista/2008)|
|SMB 3.x|Added encryption, multichannel support (Windows 8/2012+)|

#### 🔌 **Ports Used**

- **TCP 445** – Direct SMB communication (**modern usage**)
    
- **TCP 139** – SMB over NetBIOS (**legacy support**)

---

## 🎯 **SMB & NetBIOS Enumeration in Pentesting**

### 🛠️ **Why It's Important:**

- Discover **hostnames**, **usernames**, **shared resources**, and **domain info**.
    
- Attackers can use these to perform **lateral movement**, **bruteforce**, or **gain initial access**.
...
|---|---|
|`nmap`|`nmap -p 137-139,445 --script nbstat,smb-enum* <target>`|
|`enum4linux`|Legacy but still powerful for NetBIOS/SMB info gathering|
|`smbclient`|CLI access to shares: `smbclient -L //<target> -U guest`|
|`crackmapexec`|Automates many SMB-related enumeration and attacks|
|`nbtscan`|Scans NetBIOS info across subnets|

---

## 🔄 **Modern Usage Trend**

- **NetBIOS is largely deprecated** in modern networks.
    
    - DNS, mDNS, and LDAP have replaced NetBIOS for name resolution.
        
- **SMB over port 445 is now standard.**
    
    - Port 139 is **only needed for backward compatibility** (e.g., old Win XP boxes).
        
- **Disabling NetBIOS** is a **recommended hardening measure** unless needed.
...
If you’re doing internal network pentests, SMB/NetBIOS enumeration is **gold** for gathering:

- Valid users and groups
    
- Shared folders (potential sensitive data)
    
- Misconfigured permissions
    
- OS version (vuln matching)
    
- Potential credentials via SMB login bruteforce or relay attacks
...
- **Relay Attacks → NTLM relay via SMB**
    
- **Null Sessions** → Access SMB with no credentials
    
- **Misconfigured Shares** → Read/write access to sensitive paths
...
- A **cheatsheet for SMB enumeration tools**
    
- **Hands-on lab setup**
    
- Breakdown of **EternalBlue**, **SMB relays**, or **NTLM abuses**
