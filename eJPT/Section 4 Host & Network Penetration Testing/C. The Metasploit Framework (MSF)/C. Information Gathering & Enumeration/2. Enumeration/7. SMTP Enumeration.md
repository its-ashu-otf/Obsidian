...
- **What is SMTP?**
    ...
    - **SMTP (Simple Mail Transfer Protocol)** is used for **sending** emails between clients and servers, and between mail servers.
        
    - It does **not** handle email retrieval (that’s done by POP3 or IMAP).
        
    - Often an overlooked but valuable target in internal/external assessments.
        
- **Default Ports**
    
    - `TCP 25` – Standard SMTP (often unencrypted)
        
    - `TCP 465` – SMTP over SSL (SMTPS)
        
    - `TCP 587` – Submission port (STARTTLS encryption, used by clients)
...
- **Banner Grabbing**
    ...
    - Wait for the `220` banner (e.g., `220 mail.example.com ESMTP Postfix`)
        
    - Use the `EHLO` or `HELO` command to gather info:
        
        ```bash
        EHLO test.com
        ```
        
        - May return supported extensions like `PIPELINING`, `STARTTLS`, `AUTH LOGIN`
            
- **Service Detection (Nmap)**
    
    ```bash
    nmap -sV -p 25,465,587 --script smtp-commands,smtp-enum-users <target-ip>
    ```
    
    - `smtp-commands`: Lists supported SMTP commands.
        
    - `smtp-enum-users`: Bruteforce usernames using the `VRFY` or `EXPN` commands.
...
- **Common SMTP Commands**
    ...
    - `VRFY <username>` – Verifies if a user exists (if enabled)
        
    - `EXPN <list>` – Expands a mailing list (rare, but useful if enabled)
        
    - `RCPT TO:<user@example.com>` – Can be used to test for valid users when `VRFY` is disabled
        

---

### ⚠️ Misconfigurations and Weaknesses to Watch

- **Open Relays**
    
    - If the SMTP server allows unauthenticated mail forwarding, it can be abused to send spam.
...
- **Unencrypted Auth**
    ...
    - If authentication is required but **STARTTLS** isn’t enforced, credentials may be leaked in plaintext.
        
- **Verbose Error Messages**
    
    - Useful for determining user existence, software version, or security controls.
        
- **Leaky Banners**
    
    - Many SMTP daemons disclose software name/version: e.g., `Postfix 3.4`, `Exim 4.93`, `Sendmail 8.14`.
        

---

### 📌 Post-Enumeration Opportunities

- **Credential Harvesting**
    
    - If credentials are found, try to:
        
        - Access webmail interfaces
            
        - Reuse credentials on other services (especially if email = username)
            
- **Social Engineering**
    
    - Enumerated user lists can be used for phishing or spoofed spearphishing campaigns.
        
- **Pivoting**
    
    - Mail servers may serve as gateways to internal networks (especially in internal pentests).
