...
![[Pasted image 20250113010618.png]]
...
![[Pasted image 20250113005128.png]]

PSK is commonly used in home and small office Wi-Fi setups. It relies on a **single, shared password** that all devices use to connect to the network.

#### **Steps in the PSK Process:**

1. **Authentication Request**: The client (user device) sends a request to the Access Point (AP) to connect.
2. **Challenge Text**: The AP sends a challenge (a random piece of text) to the client.
3. **Encryption by the Client**: The client encrypts the challenge text using the pre-shared key (the Wi-Fi password) and sends it back.
4. **Verification by the AP**: The AP decrypts the received text using the pre-shared key. If the decrypted result matches the original challenge text, authentication is successful.
5. **Connection Established**: If the authentication is successful, the client gains access to the network.

---

### **2. Centralized Authentication Mode (RADIUS)**

![[Pasted image 20250113005145.png]]

This mode is typically used in enterprise environments where each user has unique credentials. It leverages a **RADIUS server** to authenticate users, ensuring enhanced security.

#### **Steps in the RADIUS Authentication Process:**

1. **Client Requests Connection**: The client sends a connection request to the AP.
2. **AP Requests Identity**: The AP sends an EAP (Extensible Authentication Protocol) request to determine the client’s identity.
3. **Client Responds**: The client sends its identity (e.g., username) to the AP.
4. **Identity Forwarded to RADIUS Server**: The AP forwards the client’s identity to the RADIUS server for authentication.
5. **Authentication Mechanism**: The RADIUS server sends a specific authentication mechanism request to the AP.
6. **Client Responds with Credentials**: The client provides its credentials (e.g., username and password).
7. **Authentication Key**: If the credentials are valid, the RADIUS server sends an encrypted authentication key to the AP.
8. **Session Key**: The AP uses this key to establish a secure session with the client.

---

### **Comparison Example: Home vs. Office Wi-Fi**

- **PSK Mode Example (Home)**:
    
    - The user connects to their home Wi-Fi "MyHomeWiFi" by entering a single password, such as "Home1234".
    - Devices use this shared password to authenticate with the router.
- **Centralized Mode Example (Office)**:
    
    - An employee connects to the office Wi-Fi "CompanyNetwork".
    - Instead of a shared password, the employee enters a unique username and password (e.g., "john.doe" and "password123"), which are verified by the RADIUS server.
...
1. **PSK Mode**:
    ...
    - Simpler setup for smaller networks.
    - No need for additional servers or infrastructure.
    - Less expensive.
2. **RADIUS Mode**:
    
    - Enhanced security due to individual user credentials.
    - Easier to manage access control (e.g., revoke access for a specific user without affecting others).
    - Ideal for large or sensitive environments.
...
Directional antennas are designed to focus their signal in a specific direction, providing a stronger and more concentrated signal over long distances in that direction. This makes them ideal for applications where the signal needs to reach a particular location, such as in point-to-point communication links. They are often used in scenarios where reducing interference and improving signal quality are priorities.

The different types of wireless antennas and their characteristics. Here's a brief overview of the types:

1. **Directional Antenna**:
    
    - **Description**: A directional antenna focuses the radio signal in a single direction, improving transmission and reception in that area. This design helps in reducing interference and enhances signal strength over long distances.
    - **Usage**: Often used in point-to-point communication links and in cases where the signal needs to be focused in one direction.
    
![[Pasted image 20250113005600.png]]

2. **Omnidirectional Antenna**:
    
    - **Description**: This antenna radiates electromagnetic energy uniformly in all directions, providing a 360° horizontal radiation pattern. However, the signal strength in the third dimension (vertical) is weaker.
    - **Usage**: Commonly used in scenarios where the receiver might be in various locations, such as with Wi-Fi networks or radio stations, where a broad, all-encompassing signal is needed.
    
![[Pasted image 20250113005628.png]]

3. **Parabolic Grid Antenna**:
    
    - **Description**: This antenna uses a grid instead of a solid dish, focusing the signal similarly to a satellite dish but with a lighter and more compact design. It can achieve very long-distance Wi-Fi transmission, even up to 10 miles.
    - **Usage**: Ideal for transmitting weak signals over long distances. Its focused beam is useful for attacks like man-in-the-middle (MITM) and denial-of-service (DoS) attacks, which require high gain and long-range signals.

![[Pasted image 20250113005656.png]]

4. **Yagi Antenna**:
    
    - **Description**: A unidirectional antenna with high gain, it uses a reflector, dipole, and directors to focus the radiation pattern. It provides a narrow, high-efficiency beam.
    - **Usage**: Typically used in communications within the VHF and UHF frequency bands and for applications where focused and high gain is needed.
5. **Dipole Antenna**:
    
    - **Description**: A basic antenna consisting of a straight conductor that is half the wavelength of the signal. It is bilaterally symmetrical and balanced, making it suitable for use with balanced transmission lines.
    - **Usage**: Common in many RF applications, particularly in simple and basic antenna designs, such as for local Wi-Fi access points.
6. **Reflector Antennas**:
    
    - **Description**: These antennas use parabolic reflectors to concentrate and direct radio signals. The reflector helps improve signal strength and can reduce interference, making them suitable for focused communication.
    - **Usage**: Typically used in satellite communication and other high-gain applications.

Each of these antennas has specific advantages based on the environment and the communication needs, from covering a broad area with an omnidirectional antenna to achieving long-range communication with a parabolic grid or Yagi antenna.
